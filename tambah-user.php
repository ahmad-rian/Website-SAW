...
if (isset($_POST['submit'])) :
	...
	if (empty($errors)) :
		$pass = sha1($password);
		$simpan = mysqli_query($koneksi, "INSERT INTO user (id_user, username, password, nama, email, role) VALUES (NULL, '$username', '$pass', '$nama', '$email', '$role')");
		if ($simpan) {
			echo "<script>alert('Data berhasil disimpan'); window.location='list-user.php?status=sukses-baru';</script>";
			exit();
		} else {
